...
if has_monitoring_mod then
  cache_size_metric = monitoring.gauge("digtron_customizations_cache_size", "Count of all memoized results")
end
...
function digtron_customizations.invalidate_area(pos1, pos2, margin)
...
function digtron_customizations.invalidate(pos, radius)
...
function digtron_customizations.invalidate_all()
...
function digtron_customizations.is_protected(pos, name)
...
function digtron_customizations.cache(pos, name, result)
  local mapblock_x = math.floor(pos.x / 16)
  local hash = minetest.hash_node_position(pos)
